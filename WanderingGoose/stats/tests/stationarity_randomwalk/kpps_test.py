...
def kpps_test(y: pd.Series, significance: str = "5%", test_around_trend="c") -> bool:
    ...
    if significance not in ["10%", "5%", "2.5%", "1%"]:
        raise Exception(
            "The proper significance (var: significance) level was not selected to measure the KPPS test.\n\
        It needs to be either '10%','5%','2.5%', or '1%'"
        )
    if test_around_trend not in ["c", "ct"]:
        raise Exception(
            "The proper regression to test around a trend (var: test_around_trend) was not selected.\n\
    ...
    statistic, p_value, n_lags, critical_values = kpss(
        x=y, regression="c", nlags="auto"
    )
    p_value, selected_sig = (
        critical_values[significance],
        numerical_sig_val[significance],
    )
